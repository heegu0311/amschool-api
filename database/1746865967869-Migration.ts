...
export class Migration1746865967869 implements MigrationInterface {
  name = 'Migration1746865967869';
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(`
            CREATE TABLE \`article\` (
                ...
                \`videoTag\` text NOT NULL COMMENT '영상태그',
                \`thumbnail\` varchar(50) NOT NULL COMMENT '대표이미지' DEFAULT '',
                \`adminId\` varchar(50) NULL COMMENT '아이디',
                \`author\` varchar(50) NOT NULL COMMENT '기자명' DEFAULT '',
                \`adminEmail\` varchar(50) NULL COMMENT '이메일',
                ...
                \`articleGrade\` enum ('M', 'T', 'I', 'B') NULL COMMENT '등급' DEFAULT 'B',
                \`articleType\` enum ('B', 'P', 'C', 'N', 'M', 'G') NOT NULL COMMENT '기사형태' DEFAULT 'B',
                \`views\` int UNSIGNED NOT NULL COMMENT '조회수' DEFAULT '0',
                \`serialNumber\` int UNSIGNED NOT NULL COMMENT '시리얼번호' DEFAULT '0',
                \`exclusiveUse\` enum ('Y', 'N') NOT NULL COMMENT '지면사용여부' DEFAULT 'N',
                \`printPage\` int UNSIGNED NOT NULL COMMENT '면' DEFAULT '0',
                \`status\` enum ('I', 'C', 'R', 'E') NOT NULL COMMENT '상태' DEFAULT 'I',
                \`viewRank\` enum ('A', 'M', 'C') NOT NULL COMMENT '조회등급' DEFAULT 'A',
                \`onOff\` enum ('Y', 'N') NOT NULL COMMENT '외부노출여부' DEFAULT 'N',
                \`externalExposure\` enum ('O', 'F') NOT NULL COMMENT '온/ 오프' DEFAULT 'O',
                \`publishDate\` date NOT NULL COMMENT '발행일자' DEFAULT '1000-01-01',
                \`isVisible\` enum ('Y', 'N') NOT NULL COMMENT '노출' DEFAULT 'N',
                \`mbaUsage\` enum ('Y', 'N') NOT NULL COMMENT '엠바고사용여부' DEFAULT 'N',
                \`mbaEndTime\` int UNSIGNED NOT NULL COMMENT '엠바고시간' DEFAULT '0',
